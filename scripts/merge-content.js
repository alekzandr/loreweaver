...
function detectContentType(data) {
  ...
  if (data.name && data.description) {
    if (data.resolution || data.weight !== undefined) return 'encounter';
    if (data.atmosphereEffect || data.inhabitants) return 'location';
    if (data.personality || data.motivation) return 'npc';
    if (data.DC && data.successEffect) return 'skillcheck';
    if (data.severity || data.occurrence) return 'danger';
  }
...
function isDuplicate(newContent, existingContent) {
  return existingContent.some(item => {
    // Exact name match
    if (item.name === newContent.name) return true;
...
function mergeContent(submissionPath) {
  ...
  if (isDuplicate(submission, existingContent)) {
    return { 
      ...
      warning: `Content with name "${submission.name}" already exists`
    };
  }
  ...
  if (!submission.id) {
    submission.id = generateId(submission.name, existingIds);
