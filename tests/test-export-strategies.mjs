...
function testJSONExportStrategy() {
    ...
    const testData = {
        ...
        partyLevel: '6',
        currentEncounterFlow: [{ step: 1, title: 'Start' }],
        currentEncounterLocations: [],
        currentEncounterNPCs: [],
        currentEncounterSkillChecks: [],
        currentEncounterDangers: []
    ...
    assert(parsed.encounter.title === 'Test Encounter', 'Should include title in JSON');
    assert(parsed.encounter.environment === 'Swamp', 'Should include environment in JSON');
...
function testFilenameGeneration() {
    ...
    assert(filename3.includes('encounter'), 'Should use default title');
...
function testFormatConsistency() {
    ...
    strategies.forEach(strategy => {
        ...
        if (strategy !== 'json') {
            assert(result.content.includes('Entrance') || result.content.includes('ENCOUNTER FLOW'), 
                `${strategy} should include encounter flow`);
        }
    });
